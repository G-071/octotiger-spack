...
class Octotiger(CMakePackage, CudaPackage, ROCmPackage):
    ...
    variant('simd_library', default='KOKKOS',
            description='Use either kokkos (for kokkos simd types) or std (for std::experimental::simd) for the simd types',
    ...
    for sm_ in CudaPackage.cuda_arch_values:                                                                                                                                                                                                                                
        depends_on(kokkos_string + ' +cuda +cuda_lambda +wrapper cuda_arch={0}'.format(sm_), when='+kokkos +cuda cuda_arch={0} %gcc'.format(sm_))
        depends_on(kokkos_string + ' +cuda +cuda_lambda -wrapper cuda_arch={0}'.format(sm_), when='+kokkos +cuda cuda_arch={0} %clang'.format(sm_))
    depends_on("kokkos@4.1.00: +hpx +hpx_async_dispatch ", when="+kokkos_hpx_kernels")
    ...
    #    depends_on(kokkos_string + ' +cuda +cuda_lambda +wrapper amdgpu_target={0}'.format(gfx), when='+kokkos +cuda amdgpu_target={0}'.format(gfx))
    ...
    def cmake_args(self):
        ...
        args.append(self.define_from_variant('OCTOTIGER_KOKKOS_SIMD_LIBRARY', 'simd_library'))
        args.append(self.define_from_variant('OCTOTIGER_KOKKOS_SIMD_EXTENSION', 'simd_extension'))
        args.append(self.define_from_variant('OCTOTIGER_WITH_MONOPOLE_HOST_HPX_EXECUTOR', 'kokkos_hpx_kernels'))
        args.append(self.define_from_variant('OCTOTIGER_WITH_MULTIPOLE_HOST_HPX_EXECUTOR', 'kokkos_hpx_kernels'))
        args.append(self.define_from_variant('OCTOTIGER_WITH_HYDRO_HOST_HPX_EXECUTOR', 'kokkos_hpx_kernels'))
        args.append(self.define_from_variant('OCTOTIGER_WITH_KOKKOS_MULTIPOLE_TASKS', 'multipole_host_tasks'))
        args.append(self.define_from_variant('OCTOTIGER_WITH_KOKKOS_MONOPOLE_TASKS', 'monopole_host_tasks'))
        args.append(self.define_from_variant('OCTOTIGER_WITH_KOKKOS_HYDRO_TASKS', 'hydro_host_tasks'))
